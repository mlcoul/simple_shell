...
/**
 * fd_builtin - finds a builtin command
 * @info: the parameter & return info struct
 *
...
int fd_builtin(info_t *info)
{
	...
	for (i = 0; builtintbl[i].type; i++)
		if (_strcmps(info->argv[0], builtintbl[i].type) == 0)
		{
			info->line_count++;
...
void fd_cmd(info_t *info)
{
	...
	path = f_paths(info, _getenvi(info, "PATH="), info->argv[0]);
	if (path)
	{
		info->path = path;
		fork_command(info);
	...
	{
		if ((interactive(info) || _getenvi(info, "PATH=")
...
void fork_command(info_t *info)
{
	...
	if (child_pid == 0)
	{
		if (execve(info->path, info->argv, gets_envir(info)) == -1)
		{
			free_infos(info, 1);
			if (errno == EACCES)
				exit(126);
