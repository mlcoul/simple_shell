...
{
	ssize_t fd;
	char *filename = get_hist_files(info);
	list_t *node = NULL;
...
int read_hist(info_t *info)
{
	...
	struct stat st;
	char *buf = NULL, *filename = get_hist_files(info);
	...
	for (i = 0; i < fsize; i++)
		...
		{
			buf[i] = 0;
			build_hist_lists(info, buf + last, linecount++);
			last = i + 1;
		}
	if (last != i)
		build_hist_lists(info, buf + last, linecount++);
...
/**
 * build_hist_lists - adds entry to a history linked list
...
int build_hist_lists(info_t *info, char *buf, int linecount)
{
	...
	if (info->history)
		node = info->history;
	ad_nodes_end(&node, buf, linecount);
