...
/**
 * combine_path - Combines directory and filename
 * @dir: directory to combine
 * @file: file to combine after directory
 *
 * Description: Example output: /bin/ls
 *              Where "/bin" is the dir, and "ls" is the filename
 * Return: pointer to newly allocated string containing both
 */
char *combine_path(char *dir, char *file)
{
	...
	f_len = _strlen(file);
	d_len = _strlen(dir);
	n_len = d_len + f_len + 1; /* accounting for '/' & '\0' */
