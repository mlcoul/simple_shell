...
/**
 * _mycd - changes the current direc of the process
 * @info: Struc containing potential arguments. Used to maintain
 *          constant function prototype.
 *  Return: Always 0
 */
int _mycd(info_t *info)
{
	...
	if (!info->argv[1])
	{
		...
		if (!dir)
			chdir_ret = 
				chdir((dir = _getenvi(info, "PWD=")) ? dir : "/");
		else
			chdir_ret = chdir(dir);
	}
	else if (_strcmp(info->argv[1], "-") == 0)
	{
		...
		chdir_ret = 
			chdir((dir = _getenvi(info, "OLDPWD=")) ? dir : "/");
	}
	else
	...
	if (chdir_ret == -1)
	{
		...
		_eputss(info->argv[1]), _eputchara('\n');
	}
	else
	{
